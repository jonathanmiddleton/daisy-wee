...
@pytest.mark.skip(reason="Disabled - requires changing GPTCore.forward to return logits for testing")
...
def test_step_equals_forward_when_window_covers_all(T, window):
    ...
    model = GPTCore(vocab_size, num_layers, num_heads, model_dim, max_seq_len).eval()
    device = "cpu"
    model.to(device)
