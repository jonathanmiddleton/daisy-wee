...
def effective_rank90(A_2d: torch.Tensor, device: str | torch.device) -> int:
    """A_2d: (T, T)."""
...
def columns90(A_2d: torch.Tensor, device: str | torch.device) -> int:
...
def average_per_head_over_sequences(attn: CausalSelfAttention, X_list: list[torch.Tensor], device: str | torch.device) -> dict:
    H = int(attn.num_heads)
